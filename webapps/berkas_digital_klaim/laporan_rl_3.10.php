<?php
/*
 * File: laporan_rl_3.10.php
 * Fungsi: Laporan RL 3.10 Rekapitulasi Kegiatan Pelayanan Rujukan
 */
require_once('csrf.php');
if (!isset($_SESSION['casemix_login']) || $_SESSION['casemix_login'] !== true) {
    header("Location: index.php"); exit;
}
require_once('../conf/conf.php');
$koneksi = bukakoneksi();

$q_set = mysqli_query($koneksi, "SELECT nama_instansi, logo FROM setting LIMIT 1");
$r_set = mysqli_fetch_assoc($q_set);
$nama_instansi = $r_set['nama_instansi'] ?? 'RS';
$logo_b64 = isset($r_set['logo']) ? 'data:image/jpeg;base64,' . base64_encode($r_set['logo']) : 'logo.php';
$nama_user_login = $_SESSION['namauser'] ?? "User";
$tgl_awal  = $_GET['tgl_awal'] ?? date('Y-m-01'); 
$tgl_akhir = $_GET['tgl_akhir'] ?? date('Y-m-d');
?>

<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Laporan RL 3.10 - Rujukan</title>
    <link rel="icon" href="logo.php" type="image/png">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <link href="https://cdn.datatables.net/1.13.4/css/dataTables.bootstrap5.min.css" rel="stylesheet">
    <style>
        body { background-color: #f4f6f9; font-family: 'Segoe UI', sans-serif; font-size: 0.85rem; }
        .top-navbar { background: #fff; box-shadow: 0 2px 5px rgba(0,0,0,0.05); padding: 10px 20px; }
        .table th { white-space: nowrap; text-align: center; vertical-align: middle; background-color: #f8f9fa; }
    
    </style>
</head>
<body>
<div class="d-flex" id="wrapper">
    <div id="overlay" onclick="toggleMenu()"></div>
    <?php include 'sidebar.php'; ?>
    <div id="page-content-wrapper">
        <nav class="top-navbar d-flex justify-content-between align-items-center sticky-top">
            <button class="btn btn-outline-secondary border-0" id="menu-toggle"><i class="fas fa-bars fa-lg"></i></button>
            <div class="d-flex align-items-center">
                <div class="text-end me-3 d-none d-md-block">
                    <div class="fw-bold text-dark small"><?= htmlspecialchars($nama_user_login) ?></div>
                    <small class="text-muted">Petugas Casemix</small>
                </div>
                <img src="<?= $logo_b64 ?>" class="rounded-circle border" width="35">
            </div>
        </nav>

        <div class="container-fluid px-4 py-4">
            <div class="card shadow-sm mb-4 border-0">
                <div class="card-body">
                    <h5 class="fw-bold text-primary mb-3 d-flex justify-content-between align-items-center">
                        <span><i class="fas fa-exchange-alt me-2"></i>Laporan RL 3.10 (Rekapitulasi Pelayanan Rujukan)</span>
                        <button type="button" class="btn btn-outline-info btn-sm" data-bs-toggle="modal" data-bs-target="#modalJuknis">
                            <i class="fas fa-info-circle me-1"></i> Petunjuk Teknis
                        </button>
                    </h5>
                    <form id="filterForm" class="row g-2 align-items-end">
                        <div class="col-md-3">
                            <label class="form-label small">Dari Tanggal</label>
                            <input type="date" class="form-control form-control-sm" id="tgl_awal" value="<?= $tgl_awal ?>">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label small">Sampai Tanggal</label>
                            <input type="date" class="form-control form-control-sm" id="tgl_akhir" value="<?= $tgl_akhir ?>">
                        </div>
                        <div class="col-md-2">
                            <button type="button" class="btn btn-primary btn-sm w-100" onclick="loadData()"><i class="fas fa-search me-1"></i> Tampilkan</button>
                        </div>
                    </form>
                </div>
            </div>

            <div class="card shadow border-0">
                <div class="card-body p-2">
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-sm w-100" id="dataTable">
                            <thead>
                                <tr>
                                    <th rowspan="2">No</th>
                                    <th rowspan="2" class="text-start">Jenis Spesialisasi</th>
                                    <th colspan="4" class="bg-primary text-white">Rujukan Masuk</th>
                                    <th colspan="4" class="bg-info text-white">Rujukan Masuk Dikembalikan Ke</th>
                                    <th colspan="3" class="bg-warning text-dark">Dirujuk Keluar</th>
                                    <th rowspan="2" class="bg-success text-white">Diterima Kembali</th>
                                </tr>
                                <tr>
                                    <th class="bg-primary-subtle">Pusk</th>
                                    <th class="bg-primary-subtle">RS Lain</th>
                                    <th class="bg-primary-subtle">Lainnya</th>
                                    <th class="bg-primary text-white">Total</th>
                                    <th class="bg-info-subtle">Pusk</th>
                                    <th class="bg-info-subtle">RS Asal</th>
                                    <th class="bg-info-subtle">Lainnya</th>
                                    <th class="bg-info text-white">Total</th>
                                    <th class="bg-warning-subtle text-dark">Pasien Rujukan</th>
                                    <th class="bg-warning-subtle text-dark">Dtg Sendiri</th>
                                    <th class="bg-warning text-dark">Total</th>
                                </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<!-- Modal Petunjuk Teknis Juknis -->
<div class="modal fade" id="modalJuknis" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-info text-white">
                <h5 class="modal-title"><i class="fas fa-book-open me-2"></i>Petunjuk Teknis Pengisian RL 3.10</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body" style="font-size: 0.85rem; line-height: 1.5;">
                <div class="alert alert-primary">
                    <h6><i class="fas fa-file-alt me-2"></i>Petunjuk Teknis Kemenkes (Juknis SIRS 6.3)</h6>
                    <p>Mencakup data kegiatan rujukan masuk (diterima dari Puskesmas, RS Lain, Faskes Lain) dan rujukan keluar (pasien rujukan, pasien datang sendiri). Data dilaporkan menurut jenis spesialisasi. Total rujukan masuk dikembalikan tidak boleh lebih dari total rujukan masuk, dan diterima kembali tidak boleh lebih dari total dirujuk keluar.</p>
                </div>
                <div class="alert alert-warning">
                    <h6><i class="fas fa-code me-2"></i>Logika Teknis SIMKES Khanza</h6>
                    <p>Sistem menganalisis tabel <code>rujuk_masuk</code> untuk rujukan dari faskes lain. Untuk rujukan keluar, data diambil dari tabel <code>rujuk</code> atau <code>surat_rujukan_balik</code>. Spesialisasi ditentukan berdasarkan poliklinik tujuan atau keahlian dokter yang menangani pasien saat registrasi.</p>
                    <hr>
                    <strong>Sumber Data:</strong>
                    <ul class="mb-0">
                        <li><code>rujuk_masuk</code> (Data rujukan masuk)</li>
                        <li><code>rujuk</code> (Data rujukan keluar)</li>
                        <li><code>surat_rujukan_balik</code> (Data balikan rujukan)</li>
                        <li><code>poliklinik</code> (Pemetaan jenis spesialisasi)</li>
                    </ul>
                </div>
            </div>
            <div class="modal-footer bg-light">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
            </div>
        </div>
    </div>
</div>

<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
<script src="https://cdn.datatables.net/1.13.4/js/jquery.dataTables.min.js"></script>
<script src="https://cdn.datatables.net/1.13.4/js/dataTables.bootstrap5.min.js"></script>
<script>
    var myTable;
    $(document).ready(function() {
        myTable = $('#dataTable').DataTable({
            columns: [
                { data: null, render: (d,t,r,m) => m.row + 1, className: "text-center" },
                { data: "spesialisasi" },
                { data: "masuk_pusk", className: "text-end" },
                { data: "masuk_rs", className: "text-end" },
                { data: "masuk_lain", className: "text-end" },
                { data: "masuk_total", className: "text-end fw-bold" },
                { data: "balik_pusk", className: "text-end" },
                { data: "balik_rs", className: "text-end" },
                { data: "balik_lain", className: "text-end" },
                { data: "balik_total", className: "text-end fw-bold" },
                { data: "keluar_rujukan", className: "text-end" },
                { data: "keluar_sendiri", className: "text-end" },
                { data: "keluar_total", className: "text-end fw-bold" },
                { data: "terima_kembali", className: "text-end fw-bold" }
            ],
            paging: false, info: false, searching: false
        });
        loadData();
    });

    function loadData() {
        $.ajax({
            url: 'api/data_rl_3_10.php',
            data: { tgl_awal: $('#tgl_awal').val(), tgl_akhir: $('#tgl_akhir').val() },
            success: (resp) => { myTable.clear().rows.add(resp.data).draw(); }
        });
    }
</script>
<script>
    document.getElementById("menu-toggle").onclick = function () { document.body.classList.toggle("sb-sidenav-toggled"); };
    function toggleMenu() { document.body.classList.remove("sb-sidenav-toggled"); }
</script>
</body>
</html>
