<?php
/*
 * File: erm/cetak_laporan_operasi.php
 * Fungsi: Preview Laporan Operasi (Correct Column Mapping)
 */
session_start();
...
// 1. QUERY SEMUA OPERASI
$q_ops = "SELECT * FROM operasi WHERE no_rawat = '$no_rawat' ORDER BY tgl_operasi ASC";
$res_ops = mysqli_query($koneksi, $q_ops);

if(mysqli_num_rows($res_ops) == 0) die("<div style='text-align:center;margin-top:50px'><h3>Data Operasi Tidak Ditemukan</h3></div>");
...
<head>
    <style>
        body { font-family: Tahoma, Arial, sans-serif; font-size: 10px; margin: 0; padding: 20px; background: #525659; }
        .page-container { width: 210mm; margin: 0 auto; background: white; padding: 10mm; box-shadow: 0 0 10px rgba(0,0,0,0.5); margin-bottom: 10px; }
        ...
        @media print {
            ...
            .page-container { box-shadow: none; margin: 0; padding: 0; width: 100%; page-break-after: always; }
...
<div class="no-print" style="position: fixed; top: 20px; right: 20px; z-index: 9999;">
    <button onclick="window.location.href='simpan_laporan_operasi.php?no_rawat=<?= urlencode($no_rawat) ?>'" 
...
while($d_op = mysqli_fetch_assoc($res_ops)): 
    ...
    // --- PEMETAAN TIM OPERASI SESUAI NAMA KOLOM TABLE 'OPERASI' ---
    ...
    // PERBAIKAN: perawaat_resusitas (double 'a')
    ...
    // PERBAIKAN: dokter_umum (sesuai schema)
...
?>
    <div class="page-container">
        <?php include 'layout_laporan_operasi.php'; ?>
    </div>
    <div style="height: 20px; background: #525659;" class="no-print"></div>
