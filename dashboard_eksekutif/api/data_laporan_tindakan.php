...
try {
    ...
    $params = [
        ':tgl_awal' => $tgl_awal,
        ':tgl_akhir' => $tgl_akhir,
        ':tgl_awal_op' => $tgl_awal . ' 00:00:00',
        ':tgl_akhir_op' => $tgl_akhir . ' 23:59:59'
    ];
    ...
    $where_dr = "";
    if (!empty($kd_dokter)) {
        $where_dr = " AND t.kd_dokter = :kd_dokter ";
        $params[':kd_dokter'] = $kd_dokter;
    }
    ...
    if (empty($filter_kategori) || $filter_kategori == 'Operasi') {
        ...
        if (!empty($kd_dokter)) {
            $sql_op = "
                ...
                $join_base
                WHERE t.tgl_operasi BETWEEN :tgl_awal_op AND :tgl_akhir_op $where_global
    ...
    if (!empty($queries)) {
        ...
        $stmt = $koneksi_pdo->prepare($sql_final);
        $stmt->execute($params);
        ...
        $chart_top_tindakan = array_reverse($chart_top_tindakan); // Descending
