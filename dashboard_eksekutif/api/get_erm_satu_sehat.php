...
echo "<h6>Detail Inspeksi ERM (Khanza) &middot; <strong class='text-primary'>" . htmlspecialchars($no_rawat) . "</strong></h6><hr>";

try {
    if ($modul == "Encounter") {
        $stmt = $koneksi_pdo->prepare("SELECT p.no_rkm_medis, p.nm_pasien, rp.tgl_registrasi, rp.jam_reg, poli.nm_poli, pg.nama, rp.status_bayar, rp.status_lanjut FROM reg_periksa rp INNER JOIN pasien p ON rp.no_rkm_medis=p.no_rkm_medis INNER JOIN poliklinik poli ON rp.kd_poli=poli.kd_poli INNER JOIN pegawai pg ON rp.kd_dokter=pg.nik WHERE rp.no_rawat = :no_rawat");
        ...
        $stmt = $koneksi_pdo->prepare("SELECT dp.kd_penyakit, p.nm_penyakit, dp.status FROM diagnosa_pasien dp INNER JOIN penyakit p ON dp.kd_penyakit=p.kd_penyakit WHERE dp.no_rawat = :no_rawat");
        $stmt->execute([':no_rawat' => $no_rawat]);
        ...
        $stmt = $koneksi_pdo->prepare("SELECT tgl_perawatan, jam_rawat, suhu_tubuh, tensi, nadi, respirasi, kesadaran, gcs FROM pemeriksaan_ralan WHERE no_rawat = :no_rawat");
        $stmt->execute([':no_rawat' => $no_rawat]);
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $jenis = "Rawat Jalan";
        
        if(count($results) == 0) {
            $stmt = $koneksi_pdo->prepare("SELECT tgl_perawatan, jam_rawat, suhu_tubuh, tensi, nadi, respirasi, kesadaran, gcs FROM pemeriksaan_ranap WHERE no_rawat = :no_rawat");
            $stmt->execute([':no_rawat' => $no_rawat]);
            $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $jenis = "Rawat Inap";
        }
        ...
        $stmt = $koneksi_pdo->prepare("SELECT ro.no_resep, ro.tgl_perawatan, ro.jam, d.kode_brng, d.jml, d.aturan_pakai, ob.nama_brng, smo.obat_code, smo.obat_system 
                              FROM resep_obat ro 
        ...
        $racikan = "";
        $stmt_r = $koneksi_pdo->prepare("SELECT ro.no_resep, d.nama_racik, d.jml_dr, d.aturan_pakai FROM resep_obat ro INNER JOIN resep_dokter_racikan d ON ro.no_resep=d.no_resep WHERE ro.no_rawat = :no_rawat");
        $stmt_r->execute([':no_rawat' => $no_rawat]);
        $results_r = $stmt_r->fetchAll(PDO::FETCH_ASSOC);

        if(count($results_r) > 0) {
        ...
        $stmt = $koneksi_pdo->prepare($sql_lab);
        $stmt->execute([':no_rawat' => $no_rawat]);
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        if(count($results) > 0) {
            ...
            $sql_req = "SELECT pl.noorder, pl.tgl_permintaan, pl.jam_permintaan, ppl.kd_jenis_prw, j.nm_perawatan 
                        ...
                        WHERE pl.no_rawat = :no_rawat LIMIT 50";
            $stmt_req = $koneksi_pdo->prepare($sql_req);
            $stmt_req->execute([':no_rawat' => $no_rawat]);
            $results_req = $stmt_req->fetchAll(PDO::FETCH_ASSOC);
            
            if(count($results_req) > 0) {
                echo "<table class='table table-sm table-bordered'>
                    <thead class='table-warning'><tr><th>Data Order/Permintaan Lab</th><th>Status Rilis Hasil Medis</th></tr></thead><tbody>";
                foreach($results_req as $r_req) {
        ...
        $stmt = $koneksi_pdo->prepare($sql_rad);
        $stmt->execute([':no_rawat' => $no_rawat]);
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    echo "<div class='alert alert-danger'>Terjadi kesalahan sistem saat memuat data.</div>";
}
?>