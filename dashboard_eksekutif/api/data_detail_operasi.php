<?php
/*
 * File: api/data_detail_operasi.php (SECURITY HARDENED - PDO)
...
try {
    ...
    $sql = "SELECT * FROM operasi WHERE no_rawat = :no_rawat AND tgl_operasi = :tgl_operasi";
    $stmt = $koneksi_pdo->prepare($sql);
    $stmt->execute([':no_rawat' => $no_rawat, ':tgl_operasi' => $tgl_operasi]);
    ...
    // Cari Dokter
    if (!empty($dokter_ids)) {
        $unique_dokters = array_unique($dokter_ids);
        $placeholders = str_repeat('?,', count($unique_dokters) - 1) . '?';
        $stmt = $koneksi_pdo->prepare("SELECT kd_dokter, nm_dokter FROM dokter WHERE kd_dokter IN ($placeholders)");
        $stmt->execute(array_values($unique_dokters));
    ...
    if (!empty($petugas_ids)) {
        $unique_petugas = array_unique($petugas_ids);
        $placeholders = str_repeat('?,', count($unique_petugas) - 1) . '?';
        $stmt = $koneksi_pdo->prepare("SELECT nip, nama FROM petugas WHERE nip IN ($placeholders)");
        $stmt->execute(array_values($unique_petugas));
        $res = $stmt->fetchAll(PDO::FETCH_ASSOC);
        foreach ($res as $r) {
