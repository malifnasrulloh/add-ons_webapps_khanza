...
// 4. GET FATIGUE RISK (Risiko Kelelahan)
if ($act == 'fatigue') {
    $tgl1 = isset($_GET['tgl1']) ? trim($_GET['tgl1']) : '';
    $tgl2 = isset($_GET['tgl2']) ? trim($_GET['tgl2']) : '';
    $dep = isset($_GET['dep']) ? trim($_GET['dep']) : '';
    ...
    foreach ($pegawai_list as $peg) {
        ...
        $consec = 0; $max_consec = 0; $total_night = 0;
        ...
        while ($current_date <= $end_date) {
            ...
            if (!empty($jadwal_kode) && stripos($jadwal_kode, 'Malam') !== false) {
                $total_night++;
                $consec++;
        ...
        if ($max_consec >= 2) {
            // Check late count in period
            $stmt_late = $koneksi_pdo->prepare("SELECT COUNT(*) as t FROM rekap_presensi WHERE id = :id AND jam_datang BETWEEN :tgl1 AND :tgl2 AND status LIKE '%Terlambat%'");
            $stmt_late->execute([':id' => $id_peg, ':tgl1' => $tgl1 . ' 00:00:00', ':tgl2' => $tgl2 . ' 23:59:59']);
            $late_count = (int)($stmt_late->fetch(PDO::FETCH_ASSOC)['t'] ?? 0);

            $risk = 'rendah';
            if ($max_consec >= 4 || ($max_consec >= 3 && $late_count >= 2)) $risk = 'tinggi';
            elseif ($max_consec >= 3 || $late_count >= 1) $risk = 'sedang';
    ...
    usort($fatigue_data, function($a, $b) {
        $order = ['tinggi' => 0, 'sedang' => 1, 'rendah' => 2];
        $cmp = $order[$a['risk']] <=> $order[$b['risk']];
        return $cmp !== 0 ? $cmp : ($b['max_consec'] <=> $a['max_consec']);
    });

    echo json_encode(['status' => 'success', 'data' => $fatigue_data]);
    exit;
}

// 5. GET LEMBUR COST & HOURS (Denda & Overtime)
if ($act == 'lembur') {
    $tgl1 = isset($_GET['tgl1']) ? trim($_GET['tgl1']) : '';
    $tgl2 = isset($_GET['tgl2']) ? trim($_GET['tgl2']) : '';
    $dep = isset($_GET['dep']) ? trim($_GET['dep']) : '';
    ...
    $sql = "SELECT 
                ...
                pg.nama, 
                pg.departemen, 
                COALESCE(SUM(pl.jam_lembur_aktual), 0) AS total_jam, 
                COALESCE(SUM(pl.fee_total), 0) AS total_fee,
                COUNT(pl.no_pengajuan) AS total_pengajuan
            FROM pengajuan_lembur pl
            JOIN pegawai pg ON pl.nik = pg.nik
            WHERE pl.status = 'Disetujui'
              AND pl.tanggal BETWEEN :tgl1 AND :tgl2";
    
    $params = [':tgl1' => $tgl1, ':tgl2' => $tgl2];
    if ($dep != 'ALL' && $dep != '') {
        $sql .= " AND pg.departemen = :dep";
        $params[':dep'] = $dep;
    }
    $sql .= " GROUP BY pg.nik, pg.nama, pg.departemen ORDER BY total_fee DESC";
